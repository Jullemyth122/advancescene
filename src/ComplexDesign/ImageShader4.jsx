...
float circle(vec2 uv, vec2 pos, float radius, float blur, float t) {
    ...
    vec2 v = uv - pos;
    // float d = smoothstep(pos.x, pos.y,length(v));
    float d = length(v);
...
void main() {
    ...
    float maxIter = 8.0;
    int t_iter = int(floor(t * maxIter)); // Number of recursion levels: 0, 1, 2, or 3

    if (t_iter == 0) {

        float mask = circle(vUv, vec2(0.5, 0.5), 0.4, 0.01, t);
        vec4 img = texture2D(u_texture1, vUv);
        gl_FragColor = vec4(img.rgb, mask);

    } else {

        for (int i = 0; i < t_iter; i++) {
            float mask = circle(currentUV, vec2(0.5,cos(t * 1.0)), 0.1 * sqrt(t * sin(t * 2.0)) , 0.01, t);
            finalMask = max(finalMask, mask); 
            currentUV = fract(currentUV * 2.0); 
        }

        vec4 img = texture2D(u_texture1, currentUV);
        gl_FragColor = vec4(img.rgb, finalMask);
    }
}
`;
