...
const fragmentShader = `
    ...
    void main() {
        ...
        // vec2 bl = step(vec2(sin(u_time), 0.1), st);
        // vec2 bl = smoothstep(vec2(sin(u_time), 0.1), st);

        // vec2 bl = smoothstep(vec2(sin(u_time), 1.0), vec2(0.5,0.1)), st);
        ...
        // float progress = 1.0 - abs(2.0 * fract(u_time * 0.1) - 1.0);
        // float width = 0.1;
        // float opacity = 1.0 - smoothstep(progress - width, progress, st.x);
        // gl_FragColor = vec4(vec3(1.0) * img.rgb , opacity);
        ...
        // float progress = fract(u_time);
        // float progress = 1.0 - abs(2.0 * fract(u_time * 0.5) - 1.0);
        // vec2 center = vec2(0.5, 0.5);
        // float dist = length(st - center);
        // float size = smoothstep(0.0, 1.0, progress);
        // float opacity = 1.0 - smoothstep(size - 0.1, size, dist);
